...
```
<scripttype="text/x-template"id="anchored-heading-template">
<h1 v-if="level === 1">
<slot></slot>
</h1>
...
简单清晰很多！简单来说，这样代码精简很多，但是需要非常熟悉 Vue 的实例属性。在这个例子中，你需要知道当你不使用`slot`属性向组件中传递内容时，比如`anchored-heading`中的`Hello world!`，这些子元素被存储在组件实例中的`$slots.default`中。如果你还不了解，**在深入 render 函数之前推荐阅读[实例属性 API](%24785#实例属性 "")。**


## [节点、树以及虚拟 DOM](%24860#节点、树以及虚拟-DOM "节点、树以及虚拟 DOM")<a name="节点、树以及虚拟-DOM"></a>
...
### [虚拟 DOM](%24860#虚拟-DOM "虚拟 DOM")<a name="虚拟-DOM"></a>
...
## [`createElement`参数](%24860#createElement-参数 "createElement 参数")<a name="createElement-参数"></a>
...
### [深入 data 对象](%24860#深入-data-对象 "深入 data 对象")<a name="深入-data-对象"></a>
...
### [完整示例](%24860#完整示例 "完整示例")<a name="完整示例"></a>
...
```
vargetChildrenTextContent =function(children){
returnchildren.map(function(node){
returnnode.children
...
render:function(createElement){
// 创建 kebabCase 风格的ID
varheadingId = getChildrenTextContent(this.$slots.default)
.toLowerCase()
.replace(/\W+/g,'-')
.replace(/(^\-|\-$)/g,'')
...
### [约束](%24860#约束 "约束")<a name="约束"></a>

#### VNodes 必须唯一<a name="VNodes-必须唯一"></a>
...
## [使用 JavaScript 代替模板功能](%24860#使用-JavaScript-代替模板功能 "使用 JavaScript 代替模板功能")<a name="使用-JavaScript-代替模板功能"></a>

### [`v-if`和`v-for`](%24860#v-if-和-v-for "v-if 和 v-for")<a name="v-if-和-v-for"></a>
...
returncreateElement('ul',this.items.map(function(item){
returncreateElement('li', item.name)
}))
}else{
returncreateElement('p','No items found.')
}
}
...
### [`v-model`](%24860#v-model "v-model")<a name="v-model"></a>
...
### [事件 &amp; 按键修饰符](%24860#事件-amp-按键修饰符 "事件 & 按键修饰符")<a name="事件-amp-按键修饰符"></a>
...
Keys:
`.enter`,`.13` | `if (event.keyCode !== 13) return`(change`13`to[another key code](%25076 "")for other key modifiers) 
Modifiers Keys:
...
### [插槽](%24860#插槽 "插槽")<a name="插槽"></a>


你可以从[`this.$slots`](%24785#vm-slots "")获取 VNodes 列表中的静态内容：


```
render:function(createElement){
// `<div><slot></slot></div>`
returncreateElement('div',this.$slots.default)
}
...
还可以从[`this.$scopedSlots`](%24785#vm-scopedSlots "")中获得能用作函数的作用域插槽，这个函数返回 VNodes：
...
returncreateElement('span', props.text)
}
}
})
])
...
## [JSX](%24860#JSX "JSX")<a name="JSX"></a>
...
## [函数式组件](%24860#函数式组件 "函数式组件")<a name="函数式组件"></a>


之前创建的锚点标题组件是比较简单，没有管理或者监听任何传递给他的状态，也没有生命周期方法。它只是一个接收参数的函数。
在这个例子中，我们标记组件为`functional`，这意味它是无状态 (没有[响应式数据](%24785#选项-数据 ""))，无实例 (没有`this`上下文)。
一个**函数式组件**就像这样：
...
* `slots`: 返回所有插槽的对象的函数
* `data`：传递给组件的[数据对象](%24860#深入-data-对象 "")，作为`createElement`的第二个参数传入组件
* `parent`：对父组件的引用
* `listeners`: (2.3.0+) 一个包含了所有在父组件上注册的事件侦听器的对象。这只是一个指向`data.on`的别名。
* `injections`: (2.3.0+) 如果使用了[`inject`](%24785#provide-inject "")选项，则该对象包含了应当被注入的属性。
...
render:function(createElement, context){
functionappropriateListComponent(){
varitems = context.props.items

if(items.length ===0)returnEmptyList
if(typeofitems[0] ==='object')returnTableList
if(context.props.isOrdered)returnOrderedList
...
### [向子元素或子组件传递特性和事件](%24860#向子元素或子组件传递特性和事件 "向子元素或子组件传递特性和事件")<a name="向子元素或子组件传递特性和事件"></a>
...
returncreateElement('button', context.data, context.children)
}
})
...
### [`slots()`和`children`对比](%24860#slots-和-children-对比 "slots() 和 children 对比")<a name="slots-和-children-对比"></a>
...
## [模板编译](%24860#模板编译 "模板编译")<a name="模板编译"></a>
