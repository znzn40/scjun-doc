...
```
<google-map>
<google-map-markersv-bind:places="iceCreamShops"></google-map-markers>
</google-map>
...
```
<google-map>
<google-map-regionv-bind:shape="cityBoundaries">
<google-map-markersv-bind:places="iceCreamShops"></google-map-markers>
</google-map-region>
</google-map>
...
```
varmap =this.$parent.map ||this.$parent.$parent.map
...
你可以在[API 参考文档](%24785#provide-inject "")学习更多关于依赖注入的知识。
...
想了解更多程序化侦听器的内容，请查阅[实例方法 / 事件](%24785#实例方法-事件 "")相关的 API。
...
然而，如果你已经做到了上述的事项仍然发现在极少数的情况下需要手动强制更新，那么你可以通过[`$forceUpdate`](%24785#vm-forceUpdate "")来做这件事。
