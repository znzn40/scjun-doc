...
version:0
lang:zh
rawUrl:https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Boolean/toSource
---
...
* 对于内置的[`Boolean`]4930 "此页面仍未被本地化, 期待您的翻译!")对象，toSource返回以下字符串，表示源代码不可用：
```
function Boolean() {
    [native code]
}
```
* 对于[`Boolean`]4930 "此页面仍未被本地化, 期待您的翻译!")的实例，toSource返回一个表示源代码的字符串。
