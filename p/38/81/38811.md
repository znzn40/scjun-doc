...
version:0
lang:zh
rawUrl:https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/header
---
...
* `<header>`元素不是分节内容，所以不会引入新的分节到[大纲]32709 "")中。


* <dfn>[Content categories]38970 "HTML/Content_categories")</dfn>[Flow content]38973 "HTML/Content categories#Flow content"), palpable content.
* <dfn>许可内容</dfn>[Flow content]38973 "HTML/Content categories#Flow content")，但是不包含`<header>`和[`<footer>`]38808 "HTML <footer> 元素表示最近一个章节内容或者根节点（sectioning root ）元素的页脚。一个页脚通常包含该章节作者、版权数据或者与文档相关的链接等信息。")
* <dfn>标记省略</dfn>不允许，开始标签和结束标签都不能省略。
* <dfn>父元素</dfn>任何接受[flow content]38973 "HTML/Content categories#Flow content")的元素。注意`<header>`元素不能作为[`<address>`]38785 "HTML 的<address>元素可以让作者为它最近的<article>或者<body>祖先元素提供联系信息。在后一种情况下，它应用于整个文档。"),[`<footer>`]38808 "HTML <footer> 元素表示最近一个章节内容或者根节点（sectioning root ）元素的页脚。一个页脚通常包含该章节作者、版权数据或者与文档相关的链接等信息。")或其他[`<header>`]38811 "<header>元素表示一组引导性的帮助，可能包含标题元素，也可以包含其他元素，像logo、分节头部、搜索表单等。")元素的子元素。
* <dfn>DOM interface</dfn>[`HTMLElement`]2749 "HTMLElement 接口表示所有的 HTML 元素。一些HTML元素直接实现了HTMLElement接口，其它的间接实现HTMLElement接口.")

## 属性<a name="属性"></a>


只具有[全局属性]38986 "HTML/Global attributes").
...
Specification | Status | Comment 
 ---  |  ---  |  ---  | 
[HTML Living Standard<br></br><small>&lt;header&gt;</small>]39222 "") | Living Standard |  
[HTML5<br></br><small>&lt;header&gt;</small>]39223 "") | Recommendation |  
...
* A more detailed description:[The header element | HTML5 Doctor]39224 "The header element | HTML5 Doctor")
