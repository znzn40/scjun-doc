...
version:0
lang:zh
rawUrl:https://developer.mozilla.org/en-US/docs/Web/CSS/:-moz-system-metric(touch-enabled)
---
...
The**`:-moz-system-metric(touch-enabled)`**[CSS]427 "")[pseudo-class]29702 "")will match an element if the device on which the content is being rendered offers a supported touch-screen interface.



**Note**: Since[Firefox 58]36457 ""), this pseudo-class is no longer available to web content — it is only available internally (e.g. to XUL / chrome code). See[bug 1396066]36458 "FIXED: Avoid exposing :-moz-system-metric stuff in content pages.")for more details. Use the[`-moz-touch-enabled`]36476 "https://developer.mozilla.org/En/CSS/Media_queries#-moz-touch-enabled")media query instead.
